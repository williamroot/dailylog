...
def most_popular_gender(data_list):
    ...
    if male > female:
        ...
        answer = "Equal"
    return answer
    # ANOTAÇÃO DO RODOLFO: uma outra forma de fazer isso, em uma linha, logo após a linha 147:
...
def count_types(usertype_list):
    """
    Conta os tipos e retorna uma lista com a soma de cada um
    Argumentos:
    - usertype_list (list): lista dos tipos disponíveis
    Retorna:
    - valores (int) no tipo lista (list)
    """
    customer, dependent, subscriber = 0, 0, 0
    for item in usertype_list:
        if item == 'Customer':
            customer += 1
...
users_quantity = count_types(usertype_list)
# ANOTAÇÃO DO RODOLFO: outra forma de fazer isso é usando .count() numa list comprehension:
# users_quantity = [usertype_list.count(x) for x in usertypes]
...
# ANOTAÇÃO DO RODOLFO: outra forma de fazer isso:
